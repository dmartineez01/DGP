...
class _TareaMaterialAlumnoPageState extends State<TareaMaterialAlumnoPage> {
  ...
  try {
    final taskData = await fetchAssignedTask(widget.alumnoId, widget.tarea.tipo, widget.materialAsignadaId);
...
Future<void> updateElementosTarea() async {
  ...
  currentPageIndex = _pageController.hasClients ? _pageController.page?.round() ?? 0 : 0;
  ...
  Widget _buildCompleteButton() {
    ...
    final String buttonText = isTaskCompleted ? 'Marcar como Pendiente' : 'Completar Tarea';
  ...
  Widget _buildElementoTareaCard(ElementoTarea elemento, int cantidadElemento, int index) {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          child: Image.asset(
            ...
            SizedBox(height: 20),
            ElevatedButton(
              onPressed: () => isStepCompleted ? _updateTaskStatus(index) : _updateTaskStatus(index + 1),
              child: Row(
                ...
                if (snapshot.connectionState == ConnectionState.done) {
                  if (snapshot.hasData) {
                    ...
                    WidgetsBinding.instance.addPostFrameCallback((_) {
                      if (_pageController.hasClients && currentPageIndex < snapshot.data!.length) {
                        ...
                        Expanded(
                          child: PageView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              Future<int?> cantidadFuture = obtenerCantidadElemento(elemento.id);

                              return FutureBuilder<int?>(
                                future: cantidadFuture,
                                builder: (context, cantidadSnapshot) {
                                  if (cantidadSnapshot.connectionState == ConnectionState.done) {
                                    int? cantidadElemento = cantidadSnapshot.data;
                                    return _buildElementoTareaCard(elemento, cantidadElemento ?? 0, index);
                                  } else {
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            ElevatedButton(
                              ...
                              onPressed: () {
                                if (_pageController.hasClients && _pageController.page! < snapshot.data!.length - 1) {
